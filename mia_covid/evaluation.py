...
def compute_attack_inputs(model, attack_data, ds_info):
    ...
    if len(ds_info['class_counts']) == 2:
        ...
        losses = np.array([x[0] for x in bc(constant([[y] for y in labels]),
                          constant(probs), from_logits=from_logits).numpy()])
    elif len(ds_info['class_counts']) > 2:
        scc = tf.keras.backend.sparse_categorical_crossentropy
        losses = scc(constant([[y] for y in labels]), constant(
            probs), from_logits=from_logits).numpy()
...
def evaluateModelPrivacy(model, dataset: AbstractDataset, setting: run_settings):
    ...
    print(eps, order)
    print('Achieved epsilon = %.3f with delta = %f for noise scale = %.3f' %
          (eps, setting.privacy.delta, setting.privacy.noise))
    print('\n')
    ...
    for i in range(setting.commons.mia_samplenb):
        with suppress_stdout():
            max_auc, max_adv, max_ppv, adv_atk_type = run_mia(
                model, train_attack_input, test_attack_input)
        aucs.append(max_auc)
        advs.append(max_adv)
        ppvs.append(max_ppv)

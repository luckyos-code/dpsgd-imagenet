...
from ppml_datasets.datasets import MnistDataset, Covid19RadiographyDataset
...
def runExperiment(arg_dict: Dict["str", Any]):
    ...
    if arg_dict["dataset"] == "mnist":
        dataset = MnistDataset(model_img_shape=(28, 28, 3), builds_ds_info=True,
                               batch_size=model_setting.batch_size, preprocessing_func=preprocessing_func)
    elif arg_dict["dataset"] == "covid":
        dataset = Covid19RadiographyDataset(model_img_shape=(224, 224, 3), dataset_path="data",
                                            builds_ds_info=True, batch_size=model_setting.batch_size, preprocessing_func=preprocessing_func)

    settings = run_settings(dataset=dataset, model_setting=model_setting,
                            commons=commons(), privacy=privacy(target_eps=arg_dict["eps"]))
    ...
    print('Dataset: {}, Model: {}, Epsilon: {}'.format(settings.dataset.dataset_name,
          settings.model_setting.architecture, settings.privacy.target_eps))
    ...
    print(
        f"Train: {dataset.ds_info['train_count']}, Val: {dataset.ds_info['val_count']}, Test: {dataset.ds_info['test_count']}\n")
    ...
    if dataset.variants:
        ...
        print(*[f"-- {variant['activation']}" +
              f" {variant['pretraining']}" for variant in dataset.variants], sep='\n')
        print('\n')
        ...
        if USE_WANDB:
            ...
            run_name = f"{settings.dataset.dataset_name}_eps-{settings.privacy.target_eps}_{model_info['model_name']}" if settings.privacy.target_eps else settings.dataset.dataset_name + \
                '_eps-inf_' + model_info['model_name']
        ...
        # final perfomance results roundup
        for variant, performance_result, privacy_result in zip(settings.dataset.variants, performance_results, privacy_results):
            print(
                f"{settings.model_setting.architecture} {variant['activation']} {variant['pretraining']}")
